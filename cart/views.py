...
class CartActionView(View):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            messages.error(
                request,
                "Shopping cart was not updated. Please reload the page and try again.",
            )
        ...
        quantity_form = ProductQuantityForm(
            initial={"product_id": product_id, "quantity": quantity}
        )
        ...
        content += render_to_string(
            "cart/inclusions/cart_summary.html", {"cart": cart, "hx_oob": True}, request
        )
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if not form.is_valid():
            messages.error(
                request,
                "Product was not removed from the cart. Please reload the page and try again.",
            )
            return HttpResponseNotFound()
        product_id = form.cleaned_data["product_id"]
        cart = Cart(request)
